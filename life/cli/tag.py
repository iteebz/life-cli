...
from ..ops.items import manage_tag
...
def tag(
    ...
    completed: bool = typer.Option(False, "--completed", "-c", help="Include completed items in search"),  # noqa: B008
):
    """Add, remove, or view items by tag (fuzzy match)"""
    typer.echo(manage_tag(tag_name, " ".join(args) if args else None, remove=remove, include_completed=completed))
